...
#writing to csv
outfile = open('PoliceFinal.csv','w')
writer = csv.writer(outfile)

# Write header rows
headers = ['Title', 'CallDateTime', 'AptLot', 'ExtNatureDisplayName', 'InNum', 'Timestamp', 'Latitude', 'Longitude', 'Gridloc', 'Disp']
writer.writerow(headers)

# Here we'll grab an XML feed of the data (available by clicking the "GeoRSS Feed" button on the page) rather than HTML
xml = urllib2.urlopen('https://www.como.gov/PSJC/Services/911/911dispatch/police_georss.php').read()
